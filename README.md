...
사용자는 회원가입이 되어있는 상태라고 가정하였으며, 사용자의 아이디와 비밀번호를 저장한 데이터베이스를 구성하였다. 테이블은 **투표자 테이블**, **운영자 테이블** 두개로 비밀번호는 해시화하여 저장되며 투표자 테이블은 운영자 테이블과 다르게 **단과대 정보와 투표 여부를 표시하는 column**이 존재한다. 투표 여부를 표시하는 column은 투표자가 투표를 **한번만** 할 수 있도록 하기 위함이다.
